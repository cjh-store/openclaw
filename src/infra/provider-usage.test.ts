...
describe("provider usage loading", () => {
  ...
  it("collapses duplicate provider snapshots to the best available entry", async () => {
    const mockFetch = createProviderUsageFetch(async (url, init) => {
      if (url.includes("chatgpt.com/backend-api/wham/usage")) {
        const headers = (init?.headers ?? {}) as Record<string, string>;
        if (headers.Authorization === "Bearer bad-token") {
          return makeResponse(401, { error: "expired" });
        }
        return makeResponse(200, {
          rate_limit: { primary_window: { used_percent: 12, limit_window_seconds: 10800 } },
          plan_type: "Plus",
    ...
    const summary = await loadUsageWithAuth(
      [
        { provider: "openai-codex", token: "bad-token", accountId: "acc-bad" },
        { provider: "openai-codex", token: "good-token", accountId: "acc-good" },
      ],
      mockFetch,
    );

    expect(summary.providers).toHaveLength(1);
    expect(summary.providers[0]?.provider).toBe("openai-codex");
    expect(summary.providers[0]?.error).toBeUndefined();
    expect(summary.providers[0]?.windows[0]?.label).toBe("3h");
  });
